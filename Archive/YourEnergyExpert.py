...
from Archiv.EnergyConfig import system_prompt, greeting

client = OpenAI(api_key=open("./data/key.txt", "r").read().strip())
...
def chat_with_openai(input_text):
    chat_completion = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[{"role": "user", "content": system_prompt}, *input_text],
    )
...
if "messages" not in st.session_state:
    st.session_state["messages"] = [{"role": "user", "content": greeting}]
...
where = st.sidebar.text_input(
    "Where do you want to go? Tell me as much \n "
    "as you think is of interest for me to plan your trip.\n",
    
)
duration = st.sidebar.text_input("How long will your trip be?", "")
who = st.sidebar.text_input(
    "Who are you? How many? What are your interests?\n " 
    "Tell me about yourself...", ""
)
why = st.sidebar.text_input("Why you do this trip? \n "
                            "Adventure, Relaxation, ...?", "")
price = st.sidebar.text_input(
    "How much are you willing to spend? Feel free to answer qualitatively", ""
)
...
st.session_state.messages.append(
    {
        "role": "user",
        "content": f"Where: {where}, Who: {who}, \n "
        "Duration: {duration}, Why: {why}, Price: {price}, {anything}",
    }
)
